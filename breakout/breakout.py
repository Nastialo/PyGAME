...
class Breakout(Game):
    ...
    def handle_ball_collisions(self):
        ...
        for brick in self.bricks:
            ...
            if brick.special_effect is not None:
                # Сброс эффекта , если ударился об другой эффект
    ...
    def update(self):
        ...
        if self.start_level:
            self.start_level = False
            self.show_message('Готовы?!', centralized=True)

        if not self.bricks:
            self.show_message('Урааа!!!Вы Выйграли', centralized=True)
            self.is_game_running = False
            self.game_over = True
            return
        ...
        if self.game_over:
            self.show_message('Эх! Вы Проиграли!!', centralized=True)
